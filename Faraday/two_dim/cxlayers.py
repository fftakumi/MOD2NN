...
class CxMO(tf.keras.layers.Layer):
    ...
    def call(self, x):
        phi_lim = self.get_limited_complex_faraday()
        ...
        rcp_x = rcp_x * tf.exp(1.0j * phi_lim)
        rcp_y = rcp_y * tf.exp(1.0j * phi_lim)
        lcp_x = lcp_x * tf.exp(-1.0j * phi_lim)
        lcp_y = lcp_y * tf.exp(-1.0j * phi_lim)
